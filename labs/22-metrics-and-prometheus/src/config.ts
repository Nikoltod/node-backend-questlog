...
function readNodeEnvironment(): NodeEnvironment {
  ...
  if (!allowedNodeEnvironments.includes(value as NodeEnvironment)) {
    throw new Error(
      `NODE_ENV must be one of: ${allowedNodeEnvironments.join(", ")}. Received: ${value}`,
    );
  }
...
export const config = Object.freeze({
  ...
  serviceName: process.env.SERVICE_NAME ?? "node-backend-questlog-lab22",

  nodeEnvironment: readNodeEnvironment(),
  logLevel: readLogLevels(),
